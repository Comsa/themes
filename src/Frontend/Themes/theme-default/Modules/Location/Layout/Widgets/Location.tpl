...
{option:widgetLocationItem}
	...
	<div id="map{$widgetLocationItem.id}" class="parseMap" style="height: {$widgetLocationSettings.height}px; width: 100%;"></div>
		...
		<aside id="locationSearch{$widgetLocationItem.id}" class="locationSearch">
			<form method="get" action="#">
				<p>
					...
					<span id="locationSearchError{$widgetLocationItem.id}" class="text-error" style="display: none;">{$errFieldIsRequired|ucfirst}</span>
				</p>
				<p>
					<input type="submit" id="locationSearchRequest{$widgetLocationItem.id}" name="locationSearchRequest" class="btn" value="{$lblShowDirections|ucfirst}" />
				</p>
			</form>
		</aside>
	{/option:widgetLocationSettings.directions}
